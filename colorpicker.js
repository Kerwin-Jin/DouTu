class Colorpicker {
  ...
  init(opt) {
    ...
    if (!(elem && elem.nodeType && elem.nodeType === 1)) {
      throw `Colorpicker: not found  ID:${el}  HTMLElement,not ${{}.toString.call(
        el
      )}`;
    }
    ...
    this.elem_showColor = div.getElementsByClassName(
      "colorpicker-showColor"
    )[0];
    this.elem_barPicker1 = div.getElementsByClassName(
    ...
    this.elem_showModeBtn = div.getElementsByClassName(
      "colorpicker-showModeBtn"
    )[0];
    this.elem_inputWrap = div.getElementsByClassName(
  ...
  _setPosition(x, y) {
    ...
    this.hsb.b = parseInt(
      (100 * (this.pancel_height - this.pointTop)) / this.pancel_height
    );
  ...
  _setPancelColor(h) {
    ...
    this.util.css(this.elem_colorPancel, {
      background:
        "rgba(" + rgb.r + "," + rgb.g + "," + rgb.b + "," + this.rgba.a + ")",
    });
  }
  ...
  _changeViewByHsb() {
    ...
    this.util.css(this.elem_barPicker1, {
      left:
        (this.hsb.h / 360) * this.elem_barPicker1.parentNode.offsetWidth + "px",
    });
